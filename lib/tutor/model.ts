  ...
  const messages: TutorMessage[] = [
    {
      role: 'system',
      content:
        'You are the opening intake for a live AI tutor. Return strict JSON only with keys speech, awaitMode, readyToStartLesson, topic, learnerLevel. awaitMode must be exactly "voice" or "voice_or_canvas" and nothing else. CRITICAL: Be extremely brief. One short sentence max. No lecturing. No motivational filler. Ask one question, then stop. Infer learnerLevel from wording whenever possible — do not force a questionnaire. If the learner gives topic and rough level, start immediately. If the learner asks a content question, start immediately. Set readyToStartLesson true when you know enough to teach. topic = concise normalized topic or null. learnerLevel = short phrase or null. Never mention setup, stages, forms, titles, or labels.',
    },
    {
      role: 'user',
